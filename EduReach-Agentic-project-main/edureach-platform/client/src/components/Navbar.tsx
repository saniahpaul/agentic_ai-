      ...
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <Link to="/" className="flex items-center gap-2">
          ...
          <div className="flex flex-col leading-tight">
            <span className="font-heading text-xl font-bold text-maroon">Mysore College</span>
            <span className="text-[10px] text-gray-400 tracking-widest uppercase">Est. 2005 · Karnataka</span>
          </div>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-6">
          {navLinks.map((link) => (
            <a key={link.label} href={link.href}
              className="text-gray-700 hover:text-maroon transition-colors duration-200 text-sm font-medium relative group">
              {link.label}
