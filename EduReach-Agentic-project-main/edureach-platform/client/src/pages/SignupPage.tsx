...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      login(data.token);
      toast.success("Account created! Welcome to Mysore College.");
      navigate("/");
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex">
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-cream">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-500">Join Mysore College for unlimited access to AI chat & counseling calls</p>
          </div>
      ...
      <div className="hidden lg:block lg:w-1/2 relative">
        ...
        <div className="absolute inset-0 bg-maroon/70 flex items-center justify-center">
          <div className="text-center text-white p-8">
            <div className="w-20 h-20 bg-amber-400/20 border-2 border-amber-400/50 rounded-full flex items-center justify-center mx-auto mb-4">
              <GraduationCap className="w-10 h-10 text-amber-400" />
            </div>
            <h2 className="font-heading text-4xl font-bold mb-2">Join Mysore College</h2>
            <p className="text-white/80">92% placement rate · Top recruiters · 25-acre campus</p>
            <div className="mt-6 flex justify-center gap-3 flex-wrap">
              <span className="px-3 py-1 bg-amber-400/20 border border-amber-400/30 rounded-full text-amber-300 text-xs">VTU Affiliated</span>
              <span className="px-3 py-1 bg-amber-400/20 border border-amber-400/30 rounded-full text-amber-300 text-xs">AICTE Approved</span>
              <span className="px-3 py-1 bg-amber-400/20 border border-amber-400/30 rounded-full text-amber-300 text-xs">Est. 2005</span>
            </div>
          </div>
        </div>
      </div>
